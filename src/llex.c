...
static void read_xhash(LexState* ls, int del, SemInfo* seminfo) {
  ...
  while (ls->current != del) {
    if (!zhasmore(ls->z))
      luaX_lexerror(ls, "unfinished string hash", TK_EOS);
    switch (ls->current) {
    case '\n':
    case '\r':
      luaX_lexerror(ls, "unfinished string hash", TK_XHASH);
      continue;  /* to avoid warnings */
    case '\\': {
      int c;
      next(ls);  /* do not save the `\' */
      ...
      default: {
        if (!isdigit(ls->current))
          ...
          if (c > UCHAR_MAX)
            luaX_lexerror(ls, "escape sequence too large", TK_XHASH);
          save(ls, c);
        }
        continue;
      }
